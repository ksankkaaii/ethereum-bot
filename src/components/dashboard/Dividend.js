...
const Dividend = (props) => {
	...
	const detectStreet = (level) => {
		let city = '';		
		if(level >= CityLevel.whales) city = CityName[CityLevel.whales];
		else if(level >= CityLevel.sharks) city = CityName[CityLevel.sharks];
		else if(level >= CityLevel.dolphine) city = CityName[CityLevel.dolphine];
		else if(level >= CityLevel.degen) city = CityName[CityLevel.degen];
		else if(level > 0) city = 'Holder';
		else city = '';
		return city;
	}
			...
			(async () => {
				const fetchData = async () => {
					try {
						...
						setWithdrawableRewards((Number(data[1]) / 1_000_000_000_000_000_000).toFixed(4))
						setTotalRewards(((Number(data[2]) - Number(data[1])) / 1_000_000_000_000_000_000).toFixed(4))
						setTotalRewardsWithdrawn((Number(data[4]) / 1_000_000_000_000_000_000).toFixed(4))
				...
				const getAsapBal = async () => {
					try {
						...
						setAsapBalance((Number(bal) / 1_000_000_000_000_000_000).toFixed(4))
						setDegenstreet(detectStreet((Number(bal) / 1_000_000_000_000_000_000).toFixed(4)));
		...
		<div className="flex w-full flex-1 flex-col z-10 px-6 py-8 sm:px-10 sm:py-9">
			...
			<div className="flex gap-8 pt-8 flex-col 1.5xl:flex-row">
				<div className="px-6 py-5 bg-subBg rounded-2xl w-full flex flex-col justify-between">
					...
					<div className="pt-5">
						<p className="font-semibold text-3xl">{totalRewardsWithdrawn} ETH</p>
				...
				<div className="px-6 py-5 bg-subBg rounded-2xl w-full flex flex-col justify-between">
					...
					<div className="pt-5">
						<p className="font-semibold text-3xl">{totalRewards} ETH</p>
					</div>
				...
				<div className="px-6 py-5 bg-subBg rounded-2xl w-full flex flex-col justify-between">
					...
					<div className="pt-5">
						<p className="font-semibold text-3xl">{withdrawableRewards} ETH</p>
			...
			<div className="flex gap-8 pt-8 flex-col 1.5xl:flex-row">
				...
				<div className="px-6 py-5 bg-subBg rounded-2xl w-full flex flex-col justify-between">
					...
					<div className="pt-5">
						<p className="font-semibold text-3xl">${tokenInfo?.fdv ? tokenInfo?.fdv : 0}</p>
					</div>
				</div>
				<div className="px-6 py-5 bg-subBg rounded-2xl w-full flex flex-col justify-between">
					<div className="flex justify-between gap-2 items-center flex-wrap">
					...
					<div className="pt-5">
						<p className="font-semibold text-3xl">{
							tokenInfo ? (tokenInfo.fdv/tokenInfo.priceUsd).toFixed(0) : 0
						}</p>
					</div>
				</div>
